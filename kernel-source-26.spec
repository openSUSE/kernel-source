...
first_flavor="$( cd FLAVORS ; set -- * ; echo $1 )"
make include/asm $MAKE_ARGS CONFIG_CFGNAME=\"$first_flavor\"
cat >> linux.files <<-EOF
	.config
	./include/linux/version.h
	./include/linux/autoconf.h
...
cp %{SOURCE32} $RPM_BUILD_ROOT/etc/rc.d/running-kernel
