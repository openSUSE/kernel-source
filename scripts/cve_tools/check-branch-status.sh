#!/bin/bash
# check-branch-status.sh BRANCH([-]|[sha_list_file]|[sha [sha ....]]
#
...
#
# Examples
# check-branch-status.sh # check all currently tracked CVEs in VULNS_GIT
# check-branch-status.sh - # read shas/CVEs to check from the stdin
# check-branch-status.sh file # read shas/CVEs from the given file
# check-branch-status.sh sha1 CVE1 shaN # check given shas/CVEs
...
check_blacklisted()
{
	...
	local sha="$2"
	local cve="$3"
	local blacklist_entry="$(get_blacklist_entry $branch $sha $cve)"
	[ -n "$blacklist_entry" ] && echo -n "blacklisted "
}
...
__check_applied_sha()
{
	...
	echo -n "$sha $cve ${cvss:-CVSS_unknown} "
...
check_applied_sha()
{
	...
	if [ -z "$cve" ]
	then
		echo "$1 doesn't resolve to CVE" >&2
		return
	fi
